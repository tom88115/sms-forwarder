...
public class ForwardingConfig {
    ...
    public boolean matchesContent(String content) {
        if (this.contentFilter == null || this.contentFilter.isEmpty()) {
            return true; // 没有设置过滤词，匹配所有
        }
        return content != null && content.contains(this.contentFilter);
    }
        ...
        for (Map.Entry<String, ?> entry : sharedPrefs.entrySet()) {
            ...
            if (value.charAt(0) == '{') {
                try {
                    ...
                    if (json.has(KEY_CONTENT_FILTER)) {
                        config.setContentFilter(json.getString(KEY_CONTENT_FILTER));
                    } else {
